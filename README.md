...
### Client:
Client uses 4 files: config.json, local.data, remote.data and config.data (creates these files automatically). Client will use server provided tickrate and player ID. Client also starts up your game, and runs in the background. When game is shut down, client will also shutdown. local.data and remote.data files will be read/updated TICKRATE times per second. 
...
#### remote.data:
Clients' local.data files will be combined in this file as simple as this: local.data1 + local.data2 + local.data3 etc. So every client's local.data file will be in everyone's remote.data file.
